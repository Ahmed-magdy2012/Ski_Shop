...
import { PaymentSummary } from '../../api/model/paymentSummary';
...
export class PaymentPipe implements PipeTransform {
  ...
  transform(value: ConfirmationToken['payment_method_preview'] | PaymentSummary, ...args: unknown[]): unknown {
    if (value && 'card' in value) {
      ...
      const { brand, last4, expMounth, year } = value as PaymentSummary;
      return `${brand?.toUpperCase()} **** **** **** ${last4},Exp: ${expMounth}/${year}`
    }
    else {
